...
import LaneDetail from "./publish/lane/detail";
import LaneRuleDetail from "./publish/lanerule/detail";
...
export default function Sct() {
  ...
  const [selected, setSelected] = React.useState(
    history.location.pathname.match(/^\/(\w+)/)?.[1] || defaultMenu
  );
      ...
      <Body>
        <Sider>
          <Menu collapsable theme="dark">
            {Object.keys(MenuConfig).map((item, i) =>
              MenuConfig[item].isSubMenu ? (
                <Menu.SubMenu
                  key={i}
                  title={MenuConfig[item].title}
        ...
        <Content>
          <Switch>
            ...
            <Route
              exact
              path="/service/service-auth-create"
              component={ServiceAuthCreate}
            />
            <Route
              exact
              path="/service/service-limit-create"
              component={ServiceLimitCreate}
            />
            <Route
              exact
              path="/service/service-route-create"
              component={ServiceRouteCreate}
            />
            <Route
              exact
              path="/service/fuse-create"
              component={ServiceFuseCreate}
            />
            ...
            <Route exact path="/publish/lane" component={LaneDetail} />
            <Route exact path="/publish/lane-rule" component={LaneRuleDetail} />
            <Route path="*">
              <Redirect to="/guide" />
            </Route>
          </Switch>
